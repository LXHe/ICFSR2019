...
def preprocess_data(
    Dataset, ID_num_label, Target_label, 
    ID_cat_label=None,
    random_state=21, test_size=0.3
):
    ''' values of random_state and test_size are set for train_test_split'''
    
    if ID_cat_label is not None:
        Dataset[ID_cat_label] = Dataset[ID_cat_label].apply(cvt_cat, axis=0)
        pase_dummy = pd.get_dummies(Dataset[ID_cat_label])
    else:
        pase_dummy = None
    
    X = Dataset[ID_num_label]
    ...
    X_train_stdz = ss.transform(X_train)
